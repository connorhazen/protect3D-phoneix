...
float GoICP::InnerBnB(float* maxRotDisL, TRANSNODE* nodeTransOut, bool print)
{
	int i, j;
	...
	if(print){
		cout << "test*: " << initNodeTrans.w << endl;
	}
	//
	while(1)
	{
	...
	while(1)
	{
		...
		for(j = 0; j < 8; j++)
		{
		  // Calculate the smallest rotation across each dimension
			...
			ub = InnerBnB(NULL /*Rotation Uncertainty Radius*/, &nodeTrans, print);
			...
			lb = InnerBnB(maxRotDis[nodeRot.l], NULL /*Translation Node*/, false);
